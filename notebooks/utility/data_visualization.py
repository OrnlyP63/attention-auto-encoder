...
def plot(df:pd.DataFrame, ticker:str):
  ...
  for i, col in enumerate(['CapMVRVCur', 'NVTAdj', 'returns']):
    sns.lineplot(x="time", y=col, data=df, ax=axes[i], color="royalblue", label=col + f' of {name}')
...
def plot2(df:pd.DataFrame, ticker:str):
  name = df['asset'].iloc[0]
  fig, axes = plt.subplots(3, 1, figsize=(12, 10), sharex=True)  # 3 rows, 1 column
  for i, col in enumerate(['log_nvt', 'log_mvrv', 'returns']):
    sns.lineplot(x="time", y=col, data=df, ax=axes[i], color="royalblue", label=col + f' of {name}')
